...
API_VERSION_RAG = "2024-12-01-preview"
...
def extract_sources_from_completion(completion) -> list[dict]:
    ...
    try:
        ...
        debug_print("Source extraction failed", str(e))

    return sources


async def call_llm_async_with_retry(user_id:str, user_input: str, session_id: str, max_retries: int = 3, delay: int = 2) -> tuple[str, list[dict]]:
    """
...
Remember: You are designed to be maximally helpful. Even when perfect information isn't available, guide the user toward useful insights or suggest ways to refine their search."""
    ...
    cosmos_messages = get_last_messages_for_session(user_id=user_id, session_id=session_id, limit=5)
    # print("Cosmos Messages length:", len(cosmos_messages))
    if cosmos_messages:
    ...
    for attempt in range(max_retries):
        try:
                ...
                extra_body={
                    "data_sources": [{
                        ...
                        "parameters": {
                            ...
                            "semantic_configuration": "pr2semantic",
                            "authentication": {
                                "type": "api_key",
                                "key": f"{search_key}"
                            },
            ...
            # debug_file_name = f"azure_openai_raw_completion_{timestamp}.json"

            # with open(debug_file_name, "w", encoding="utf-8") as f:
            #     json.dump(raw_completion, f, indent=2, ensure_ascii=False)

            # print(f"Saved full completion to {debug_file_name}")
            ...
            if attempt < max_retries - 1 and is_genuinely_weak:
                last_response = (response_content, sources)
